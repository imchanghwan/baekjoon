...
/*
문제 풀이 아이디어
void타입 함수에 참조 인수로 값을 전달하여 병사들의 합을 센다.
N * M 번 반복하면서 각 인덱스에 해당하는 인접한 병사들의 합을 다 구했으면
다음 배열의 인덱스로 넘어가는 과정을 반복하게 되기 때문에 인수에 넣을 변수를 0으로 초기화 시켜준다.

그러고 그 합을 제곱하여 출력값에 더해준다.
...
int main()
{
	...
	for (int i = 0; i < M; i++) {
		for (int j = 0; j < N; j++) {
			...
			dfs(i, j, 'W', white); // 인접한 요소의 합을 white 인수에 참조로 전달한다팀 병사도 마찬가지로 똑같이 해준다.
			int blue = 0;
			dfs(i, j, 'B', blue);
